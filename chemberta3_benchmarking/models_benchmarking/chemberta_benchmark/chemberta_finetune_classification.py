...
def setup_logging(dataset: str, epochs: int, batch_size: int) -> logging.Logger:
    ...
    log_path = os.path.join(
        log_dir, f"chemberta_molformer_splits_run_{dataset}_epochs{epochs}_batch_size{batch_size}_{datetime_str}.log"
    )
...
def model_fn(
    ...
    """
    n_tasks = len(tasks)
    kwargs = {"num_labels": n_tasks} if n_tasks > 1 else {}
...
def main():
    ...
    for dataset in classification_datasets:
        ...
        logger = setup_logging(dataset=dataset, epochs=args.epochs, batch_size=args.batch_size)
        logger.info(f"Running benchmark for dataset: {dataset}")

        tasks = task_dict[dataset]
        logger.info(f"dataset: {dataset}, tasks: {tasks}, epochs: {args.epochs}")
