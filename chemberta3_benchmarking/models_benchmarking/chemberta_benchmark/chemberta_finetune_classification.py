...
def model_fn(tasks, model_dir, learning_rate, batch_size, pretrained_model_path): 
    # training model
    
    finetune_model = Chemberta(task='classification', learning_rate=learning_rate, batch_size=batch_size, n_tasks=len(tasks), model_dir=model_dir)
...
def run_deepchem_experiment(run_id, 
    ...
    set_seed(run_id)
    model = model_fn(tasks, model_dir, batch_size, learning_rate=learning_rate, pretrained_model_path=pretrained_model_path)
    best_score = -np.inf
