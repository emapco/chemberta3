...
def setup_logging(dataset, epochs, batch_size):
    log_dir = 'logs_chemberta'
    os.makedirs(log_dir, exist_ok=True)
    datetime_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = os.path.join(log_dir, f"chemberta_molformer_splits_run_{dataset}_epochs{epochs}_batch_size{batch_size}_{datetime_str}.log")
...
def model_fn(tasks, model_dir, batch_size, learning_rate, pretrained_model_path): 
    # training model
    
    finetune_model = Chemberta(task='regression', learning_rate=learning_rate, batch_size=batch_size, n_tasks=len(tasks), model_dir=model_dir)
    finetune_model.load_from_pretrained(pretrained_model_path)
    return finetune_model


def run_deepchem_experiment(run_id, 
                            model_fn, 
                            train_dataset, 
                            valid_dataset, 
                            test_dataset, 
    ...
    set_seed(run_id)
    model = model_fn(tasks, model_dir, batch_size, learning_rate, pretrained_model_path)
    best_score = np.inf
...
def triplicate_benchmark_dc(dataset,
                            splits_name,
                            model_fn, 
                            metric, 
                            tasks, 
                            ...
                            logger=None):
    scores = []
    train_dataset = dc.data.DiskDataset(f'../finetune_data/{splits_name}/dummy_featurized/{dataset}/train')
    valid_dataset = dc.data.DiskDataset(f'../finetune_data/{splits_name}/dummy_featurized/{dataset}/valid')
    test_dataset = dc.data.DiskDataset(f'../finetune_data/{splits_name}/dummy_featurized/{dataset}/test')

    for run_id in range(3):
        current_datetime = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        logger.info(f"Starting triplicate run {run_id + 1} for dataset {dataset} at {current_datetime}")
        model_dir = f'chemberta_model_dir_{dataset}_{run_id}_{current_datetime}'
        test_score = run_deepchem_experiment(
            run_id, model_fn,
            train_dataset, valid_dataset, test_dataset,
            metric, dataset=dataset, tasks=tasks, model_dir=model_dir,
            batch_size=batch_size, learning_rate=learning_rate,
            pretrained_model_path=pretrained_model_path, epochs=nb_epoch,
            logger=logger
        )
...
def main():
    ...
    datasets = margs.datasets
    datasets = args.dataset.split(',')
    learning_rate = margs.learning_rate
    splits_name = margs.splits_name
    ...
    pretrained_model_path = margs.pretrained_model_path
    ...
    task_dict = {'esol': ['measured_log_solubility_in_mols_per_litre'],
                'freesolv': ['expt'],
                'lipo': ['y'],
                }
    ...
    for dataset in regression_datasets:
        ...
        logger = setup_logging(dataset=dataset, epochs=epochs, batch_size=batch_size)
