...
def setup_logging(dataset: str, epochs: int,
    ...
    log_path = os.path.join(
        log_dir,
        f"dmpnn_molformer_splits_run_{dataset}_epochs{epochs}_batch_size{batch_size}_{datetime_str}.log")
...
def triplicate_benchmark_dc(
    ...
    for run_id in range(3):
        ...
        dmpnn_model_dir = 'dmpnn_model_dir'
        os.makedirs(dmpnn_model_dir, exist_ok=True)
        model_dir = f'./dmpnn_model_dir/dmpnn_model_dir_{dataset}_{run_id}_{current_datetime}'
...
def main():
    """Main function to run the DMPNN classification benchmark."""
