...
def setup_logging(dataset: str, splits_name: str, epochs: int,
    ...
    log_path = os.path.join(
        log_dir,
        f"infomax3d_{splits_name}_run_{dataset}_epochs{epochs}_batch_size{batch_size}_{datetime_str}.log"
    )

    logger = logging.getLogger(
        f"logs_infomax3d_{dataset}_epochs{epochs}_batch_size{batch_size}")
...
def model_fn(tasks: List, model_dir: str, batch_size: int, hidden_dim: int,
             target_dim: int, pretrained_model_path: str,
             learning_rate: float) -> InfoMax3DModular:
    """
    Create a Infomax3D model for classification finetune task.
    ...
    finetune_model = InfoMax3DModular(task='classification',
                                      n_tasks=len(tasks),
                                      n_classes=2,
                                      hidden_dim=hidden_dim,
                                      target_dim=target_dim,
...
def triplicate_benchmark_dc(dataset: str, splits_name: str, model_fn: Callable,
    ...
    scores = []
    train_dataset = dc.data.DiskDataset(
        f'../../data/featurized_datasets/{splits_name}/rdkit_conformer_featurized/{dataset}/train'
    )
    valid_dataset = dc.data.DiskDataset(
        f'../../data/featurized_datasets/{splits_name}/rdkit_conformer_featurized/{dataset}/valid'
    )
    test_dataset = dc.data.DiskDataset(
        f'../../data/featurized_datasets/{splits_name}/rdkit_conformer_featurized/{dataset}/test'
    )
...
def main():
    ...
    argparser.add_argument('--target_dim', type=int, default=10)
    ...
    for dataset in classification_datasets:
        ...
        logger.info(
            f"dataset: {dataset}, tasks: {tasks}, epochs: {args.epochs}, pretrained_model_path: {args.pretrained_model_path}\
learning_rate: {args.learning_rate}, batch_size: {batch_size}, splits_name: {args.splits_name}, hidden_dim: {args.hidden_dim}, target_dim: {args.target_dim}"
        )
